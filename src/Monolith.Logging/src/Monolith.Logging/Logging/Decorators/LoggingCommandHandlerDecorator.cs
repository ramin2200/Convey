...
using Monolith.Context;

namespace Monolith.Logging.Logging.Decorators;

[Convey.Types.Decorator]
internal sealed class LoggingCommandHandlerDecorator<T> : ICommandHandler<T> where T : class, ICommand
{
    ...
    {
        var module = "Todo"; // command.GetModuleName();
        var name = command.GetType().Name.Underscore();
        var messageContext = "Todo"; // _messageContextProvider.Get(command);
        var requestId = _context.RequestId;
        var traceId = _context.TraceId;
        var userId = _context.Identity?.Id;
        var messageId = "Todo"; // messageContext?.MessageId;
        var correlationId = "Todo"; // messageContext?.Context.CorrelationId ?? _context.CorrelationId;
